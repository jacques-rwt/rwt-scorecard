...
function showResults(){
  ...
  // Make the parent flex container responsive on mobile
  const ctaParent=document.querySelector('#cta-preview-cards')?.parentElement;
  if(ctaParent && window.innerWidth<768){
    ctaParent.style.flexDirection='column';
    ctaParent.style.alignItems='flex-start';
    ctaParent.style.padding='28px 20px';
    ctaParent.style.gap='24px';
  }
  // Make preview cards full width on mobile
  const previewEl=document.getElementById('cta-preview-cards');
  if(previewEl && window.innerWidth<768){
    previewEl.style.width='100%';
    previewEl.style.flexShrink='1';
    // Also fix the text column (sibling div) to be full width
    const textCol=previewEl.previousElementSibling;
    if(textCol){
  ...
  setTimeout(()=>{
    ...
    new Chart(ctx,{type:'bar',data:{labels:CATEGORIES.map(c=>{
      if(isMobile){
        // On mobile use short 2-word labels to avoid overlap
        const words=c.name.split(' ');
        return words.slice(0,2).join(' ')+(words.length>2?'…':'');
      }
      const words=c.name.split(' ');const lines=[];let cur='';words.forEach(w=>{if((cur+' '+w).trim().length>18&&cur.length>0){lines.push(cur.trim());cur=w;}else{cur=(cur+' '+w).trim();}});if(cur)lines.push(cur.trim());return lines;
    }),datasets:[{label:'Score',data:CATEGORIES.map((c,i)=>getCatScore(i)),backgroundColor:CATEGORIES.map(c=>c.color+'cc'),borderColor:CATEGORIES.map(c=>c.color),borderWidth:2,borderRadius:6}]},options:{responsive:true,maintainAspectRatio:true,clip:false,plugins:{legend:{display:false},tooltip:{callbacks:{label:ctx=>` Score: ${ctx.raw}/100`,title:items=>{const idx=items[0].dataIndex;return CATEGORIES[idx].name;}}}},scales:{y:{min:0,max:100,beginAtZero:true,ticks:{stepSize:25,font:{family:'DM Sans'}},grid:{color:'#f3f4f6'}},x:{ticks:{font:{family:'DM Sans',size:isMobile?9:11},maxRotation:isMobile?35:0,minRotation:isMobile?35:0,autoSkip:false},grid:{display:false}}},layout:{padding:{bottom:isMobile?20:10,left:0,right:16,top:0}}}});
  },100);
}
...
function saveAsPDF(){
  ...
  // Load an image src into a canvas JPEG - works for both SVG and PNG/JPEG
  function imgSrcToJpeg(src,w,h,bgColor,quality){
  ...
  var whiteLogoEl=document.querySelector('.results-header img[alt="RWT Capital"]');
  var footerLogoEl=document.querySelector('.results-body img[alt="RWT Capital"]');
  var whiteLogoSrc=whiteLogoEl?whiteLogoEl.src:null;
  var footerLogoSrc=footerLogoEl?footerLogoEl.src:null;
  ...
  Promise.all([
    imgSrcToJpeg(whiteLogoSrc,420,240,'#1e2c4d',0.85),
    imgSrcToJpeg(footerLogoSrc,320,180,'#ffffff',0.85)
  ]).then(function(logos){
  var whiteLogo=logos[0];var footerLogo=logos[1];
  var headerH=95;
